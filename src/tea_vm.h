...
static TEA_INLINE void tea_vm_push(tea_State* T, TValue value)
{
    *T->top = value;
    T->top++;
}

static TEA_INLINE TValue tea_vm_pop(tea_State* T, int n)
{
    T->top -= n;
    return *T->top;
}

static TEA_INLINE TValue tea_vm_peek(tea_State* T, int distance)
{
