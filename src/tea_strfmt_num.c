...
{
    size_t t = ((tea_fls(x | 1) * 77) >> 8) + 1; /* 2^8/77 is roughly log2(10) */
...
static char* strfmt_wuint9(char* p, uint32_t u)
{
...
static uint32_t nd_div2k(uint32_t* nd, uint32_t ndhi, uint32_t k, SFormat sf)
{
    ...
    if(k > 18)
    {
        ...
        {
            int32_t floorlog2 = ndhi * 29 + tea_fls(nd[ndhi]) - k;
...
static uint32_t nd_add_m10e(uint32_t* nd, uint32_t ndhi, uint8_t m, int32_t e)
{
    ...
    for(;;)
    {
        ...
        if(TEA_UNLIKELY(val >= 1000000000))
        {
            val -= 1000000000;
            nd[i] = val;
            if(TEA_UNLIKELY(i == ndhi))
            {
                ndhi = (ndhi + 1) & 0x3f;
                nd[ndhi] = 1;
                break;
...
static char* strfmt_wfnum(tea_State* T, SBuf* sb, SFormat sf, double n, char* p)
{
    size_t width = STRFMT_WIDTH(sf), prec = STRFMT_PREC(sf), len;
    NumberBits t;
    t.n = n;
    if(TEA_UNLIKELY((t.u32.hi << 1) >= 0xffe00000))
    {
    ...
    else if(STRFMT_FP(sf) == STRFMT_FP(STRFMT_T_FP_A))
    {
        ...
        else if(t.u32.lo | t.u32.hi)
        {
            ...
            uint32_t shift = t.u32.hi ? 20 - tea_fls(t.u32.hi) : 52 - tea_fls(t.u32.lo);
            e = -1022 - shift;
            t.u64 <<= shift;
        }
        /* abs(n) == t.u64 * 2^(e - 52) */
        ...
        if((int32_t)prec < 0)
        {
            ...
            prec = t.u32.lo ? 13 - tea_ffs(t.u32.lo) / 4 : 5 - tea_ffs(t.u32.hi | 0x100000) / 4;
        }
        else if(prec < 13)
        {
...
#error "ND_MUL2K_MAX_SHIFT too small"
        ...
        if((sf & STRFMT_T_FP_E))
        {
            ...
            if(ndebias)
            {
                ...
                */
                ...
                tea_assertT(0 <= eidx && eidx < 128, "bad eidx %d", eidx);
                nd[33] = nd[ndhi];
                nd[32] = nd[(ndhi - 1) & 0x3f];
                nd[31] = nd[(ndhi - 2) & 0x3f];
                nd_add_m10e(nd, ndhi, (uint8_t)*m_e, m_e[1]);
                if(TEA_UNLIKELY(!nd_similar(nd, ndhi, nd + 33, hilen, prec + 1)))
                {
                    goto rescale_failed;
                }
            }
            ...
            if(!p)
                p = tea_buf_more(T, sb, (width > len ? width : len) + 5);
            if(!(sf & (STRFMT_F_LEFT | STRFMT_F_ZERO)))
            {
                while (width-- > len)
                    *p++ = ' ';
        ...
        g_format_like_f:
            ...
            if(!p)
                p = tea_buf_more(T, sb, (width > len ? width : len) + 8);
            if(!(sf & (STRFMT_F_LEFT | STRFMT_F_ZERO)))
            {
                while(width-- > len)
                    *p++ = ' ';
...
SBuf* tea_strfmt_putfnum(tea_State* T, SBuf* sb, SFormat sf, double n)
{
    sb->w = strfmt_wfnum(T, sb, sf, n, NULL);
    return sb;
}
